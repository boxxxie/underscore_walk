...
test("numbers to strings", 
	 ...
	 var pre = _.walk.pre(_.clone(testTree),t);
	 var post = _.walk.post(_.clone(testTree),t);
...
test("numbers to arrays", 
	 ...
	 var pre = _.walk.pre(testTree,t);
	 // log(pre);
	 var post = _.walk.post(testTree,t);
	 // log(answer);
	 return (_.isEqual(answer,post) && _.isEqual(pre,answer));
     });
...
test("merge child with parent", 
	 ...
	 var pre = _.walk.pre(_.walk.pre(_.clone(testTree),t),t2);
	 var post = _.walk.post(_.walk.post(_.clone(testTree),t),t2);
	 return (_.isEqual(answer,post) && _.isEqual(pre,answer));
     });
...
test("general merge child with parent", 
	 ...
	 function t(node){
		 ...
		 return node.ts.map(extendFromParentToChild(parentWithoutChild));
	     }
	     return node;
	 }
	 function t2(node){
	 ...
	 var pre = _.compose(_.walk.pre_f(t2),
			     _.walk.pre_f(t))(_.clone(testTree));
	 var post = _.compose(_.walk.post_f(t2),
			      _.walk.post_f(t))(_.clone(testTree));
	 return (_.isEqual(answer,post) && _.isEqual(pre,answer));
     });
